...
async function downloadTelegramGroupMembers(format) {
  const response = await fetch(`${API}/sources/telegram/groups/exports/members.${format}`, {
    credentials: "include",
    cache: "no-store",
  });
  if (!response.ok) throw new Error("دریافت خروجی اعضای گروه‌ها ناموفق بود.");
  const blob = await response.blob();
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `telegram-members.${format}`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
async function renderTelegramGroups() {
  setHeader("تلگرام", "استخراج اعضای گروه‌ها", "مدیریت مستقل منابع، اسکن و خروجی اعضای قابل‌مشاهده");
  content.innerHTML = `<div class="loading">در حال دریافت منابع تلگرام…</div>`;
  try {
    ...
    content.innerHTML = `
      <div class="telegram-layout">
        <section class="telegram-metrics">
          <article class="telegram-metric"><span>منابع فعال</span><strong>${formatNumber(sources.length)}</strong></article>
          <article class="telegram-metric"><span>عضویت‌های قابل‌مشاهده</span><strong>${formatNumber(visibleTotal)}</strong></article>
          <article class="telegram-metric"><span>دارای یوزرنیم</span><strong>${formatNumber(usernameTotal)}</strong></article>
          <article class="telegram-metric"><span>دارای شماره</span><strong>${formatNumber(phoneTotal)}</strong></article>
        </section>
        <div class="two-column">
          <section class="panel">
            <div class="panel-header"><div><h2>افزودن گروه تلگرام</h2><p>لینک عمومی یا username گروه را وارد کنید؛ اسکن به‌صورت خودکار در صف قرار می‌گیرد.</p></div></div>
            <form id="telegram-group-form">
              <div class="form-grid">
                <label>لینک یا شناسه گروه<input name="link" dir="ltr" placeholder="https://t.me/example_group" required /></label>
                <label>دسته<select name="category"><option value="supermarket">سوپرمارکت / خرده‌فروش</option><option value="wholesaler">عمده‌فروش</option></select></label>
              </div>
              <div class="form-actions"><button class="button primary" type="submit">افزودن و شروع استخراج</button><button class="button secondary" id="scan-telegram-groups" type="button">اسکن مجدد همه</button></div>
            </form>
            <p class="privacy-note">فقط اطلاعاتی ذخیره می‌شود که Telegram برای حساب متصل قابل مشاهده کرده باشد.</p>
          </section>
          <section class="panel">
            <div class="panel-header"><div><h2>خروجی اعضا</h2><p>تمام عضویت‌های فعال همراه گروه مبدأ و اطلاعات قابل‌نمایش</p></div></div>
            <div class="telegram-actions">
              <button class="button primary" data-group-export="xlsx">دانلود Excel</button>
              <button class="button secondary" data-group-export="csv">دانلود CSV</button>
              <button class="button secondary" data-group-export="json">دانلود JSON</button>
            </div>
          </section>
        </div>
        <section class="panel">
    ...
    document.querySelector("#telegram-group-form").addEventListener("submit", async (event) => {
      event.preventDefault();
      const form = event.currentTarget;
      const button = form.querySelector("button[type='submit']");
      button.disabled = true;
      try {
        ...
        await request("/sources/telegram/groups", {
          method: "POST",
          body: JSON.stringify({link: values.get("link"), category: values.get("category")}),
        });
        showToast("گروه ثبت شد و استخراج در صف قرار گرفت.");
        await renderTelegramGroups();
      } catch (error) {
        showToast(error.message, true);
        button.disabled = false;
      }
    });
    document.querySelector("#scan-telegram-groups").addEventListener("click", async () => {
      await request("/sources/telegram/groups/scan", {method: "POST"});
      showToast("اسکن مجدد همه گروه‌ها در صف قرار گرفت.");
      await renderTelegramGroups();
    });
    document.querySelectorAll("[data-group-export]").forEach((button) => button.addEventListener("click", async () => {
      button.disabled = true;
      try {
        await downloadTelegramGroupMembers(button.dataset.groupExport);
      } catch (error) {
        showToast(error.message, true);
      } finally {
        button.disabled = false;
...
async function switchView(view) {
  ...
  if (view === "telegram-groups") return renderTelegramGroups();
