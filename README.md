...
| مورد | وضعیت |
| --- | --- |
| اینستاگرام | `instagram.com/enigsell_agency` |
| لینکدین | `linkedin.com/company/enigsell` |
| شماره تماس | **حذف شده** — در انتظار شماره درست |
| واتس‌اپ | **حذف شده** — از همان شماره ساخته می‌شد |
| تلگرام | **حذف شده** — از همان شماره ساخته می‌شد |

### وقتی شماره درست مشخص شد

سه چیز باید برگردد (کامیت `5abdc9c` همه را دارد):

1. لینک `.phone` در هدر
2. آیکون‌های تلگرام و واتس‌اپ در `.socials`
3. بلوک `.direct-call` و `.or-divider` در بخش تماس

و رول موقت انتهای `styles.css` (`.contact-choices{grid-template-columns:1fr}`)
باید حذف شود تا چیدمان سه‌ستونه برگردد. جای هرکدام در `index.html` با
کامنت `<!-- TODO -->` علامت خورده است.

سپس `./bump-assets.sh` را اجرا کنید.
...
GitHub Pages فایل‌های استاتیک را با `max-age=14400` (چهار ساعت) و HTML را با
`max-age=600` (ده دقیقه) سرو می‌کند. یعنی بازدیدکننده می‌تواند HTML جدید را با
`app.js` چهار ساعت پیش اجرا کند — و فرم خراب به نظر برسد.

برای همین آدرس `styles.css` و `app.js` در `index.html` با هش محتوا نسخه‌گذاری
شده‌اند. **بعد از هر تغییر در این دو فایل، قبل از commit این را اجرا کنید:**

```bash
./bump-assets.sh
```

هش‌ها را دوباره حساب می‌کند و `index.html` را به‌روز می‌کند. اگر فراموش شود،
کاربرانی که قبلاً سایت را دیده‌اند تا چهار ساعت نسخه قدیمی را می‌بینند.
